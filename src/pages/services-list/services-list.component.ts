    ...
    <div class="hidden print:block print:bg-white print:text-black print:p-8">
      @if (printGroup(); as group) {
            ...
            <div class="break-inside-avoid border border-gray-300 rounded-lg p-4 shadow-sm">
              ...
              @if (culto.vocals && culto.vocals.length > 0) {
                 <div class="mb-3 text-sm">
                   <span class="font-bold">🎤 VOCAL:</span> {{ culto.vocals.join(', ') }}
                 </div>
              } @else {
        ...
        <div class="mt-8 text-center text-xs text-gray-400 border-t pt-4">
          Gerado pelo Sistema de Louvores
        </div>
      }
    </div>
  `,
...
export class ServicesListComponent {
  ...
  // Sinal para guardar qual grupo (mês) será impresso
  ...
  // Helper para pegar detalhes da música no template de impressão
  getSongDetails(id: string) {
    return this.songService.songs().find(s => s.id === id);
  }
  ...
  shareMonth(group: any) {
    ...
    text += `_PIB CROATÁ_\n\n`;
    text += `🔗 *Acesse os detalhes no sistema:*\n${window.location.origin}\n`;
